...
// ── Status badge pre-compute (for template) ───────────────────────────
const statusBadgeKeys = ['available','checked_in','occupied','paid','unpaid','dirty','maintenance','checkout','cancelled'] as const
const statusBadges = statusBadgeKeys.map((s) => ({
  key: s,
  color: useStatusColor(s).name,
  label: useStatusLabel(s),
}))
...
// ── Chart 1: Revenue comparison (line) — period-aware granularity ─────
...
// ── Chart 2: Bookings (bar) — period-aware granularity ─────────────────
...
// ── Chart 3: Room status (donut) — all statuses + total center ────────
...
// ── Chart 4: Bookings by city (pie) — period-aware──────────────────────
...
// ── Chart 5: Occupancy trend (area line) — period-aware ────────────────
...
// ── Chart 6: Revenue by room type (horizontal bar) — period-aware ──────
  ...
  <div class="space-y-6">
    ...
    <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
      ...
      <div class="flex flex-wrap items-center gap-3">
        ...
        <!-- Date range picker (Popover + Calendar with range) -->
    ...
    <!-- Row 1: Revenue comparison line chart — full width -->
    <UCard>
      <template #header>
    ...
    <!-- Row 2: Monthly bookings bar + Room status donut — 2 col -->
    ...
    <!-- Row 3: Bookings by city pie + Occupancy area — 2 col -->
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <!-- Pie: Bookings by city -->
      <UCard>
        <template #header>
          <div class="flex items-center justify-between">
    ...
    <!-- Row 4: Revenue by room type — full width horizontal bar -->
    <UCard>
      <template #header>
        <div class="flex items-center justify-between">
    ...
    <UCard>
      <template #header>
        <h2 class="font-semibold text-lg">Status Colors</h2>
      </template>
    ...
    <!-- EmptyState Variants -->
    <div class="space-y-4">
      <h2 class="font-semibold text-lg">Empty State Variants</h2>
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <EmptyState
          title="Rooms"
          description="Add rooms to make them available for booking."
          icon="i-heroicons-key"
        />
        <EmptyState
          title="Guests"
          description="Once you create bookings, guests will appear here."
          icon="i-heroicons-users"
          action-label="Add Guest"
        />
      </div>
    </div>

    <footer class="text-center text-xs text-muted pb-4">
      Built with
      <a href="https://ui.nuxt.com" class="text-primary hover:underline" target="_blank">@nuxt/ui</a>
      · Powered by
      <a href="https://echarts.apache.org" class="text-primary hover:underline" target="_blank">Apache ECharts</a>
    </footer>
  </div>
</template>