...
const columns: TableColumn<any>[] = [
  { id: 'name', accessorKey: 'name', header: 'City', cell: ({ row }) => h('div', [
    h('div', { class: 'font-semibold' }, row.original.name),
    h('div', { class: 'text-xs text-muted' }, row.original.stateName || '—'),
  ]) },
...
async function load() {
  ...
  try {
    ...
    const res: any = await api.listCities(params)
    rows.value = res?.cities ?? []
    total.value = res?.meta?.pagination?.total ?? 0
  } catch (e: any) {
    toast.add({ title: 'Failed to load cities', description: e?.data?.message || e?.message, color: 'error' })
    rows.value = []
    total.value = 0
  } finally {
    loading.value = false
...
<template>
  <div class="p-4 sm:p-6 lg:p-10 space-y-5 sm:space-y-6 max-w-[1400px]">
    <header>
      ...
      <h1 class="text-xl sm:text-2xl font-bold tracking-tight">Cities</h1>
      <p class="text-sm text-muted mt-1">Every city with its hotel count.</p>
    </header>
